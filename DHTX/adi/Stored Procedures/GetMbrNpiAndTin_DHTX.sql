...
CREATE PROCEDURE [adi].[GetMbrNpiAndTin_DHTX] (@DataDate DATE, @RowStatus INT,@ClientKey INT) -- [adi].[GetMbrNpiAndTin_DHTX]'2021-06-01',0,11

AS
					-- DECLARE @DataDate DATE = '2021-06-01' DECLARE @RowStatus INT = 0 DECLARE @ClientKey INT = 11
					...
					SELECT	pr.NPI
							,pr.AttribTIN
							,src.PcpNpi			AS	ClientNPI
							,src.PcpTIN	AS	ClientTIN 
							,src.MemberID 
					FROM		(SELECT		DISTINCT m.MemberID,m.PcpNpi,M.DataDate
											,REPLACE(m.PcpTIN,'-','')PcpTIN
					...
					ON			pr.NPI = src.PcpNpi 
					AND			pr.AttribTIN = src.PcpTIN
