...
BEGIN TRAN
					...
					DECLARE @SrcName VARCHAR(100) = '[ACECAREDW].[ast].[ast.MbrStg2_MbrData]'
					...
					FROM	ACECAREDW.ast.MbrStg2_MbrData stg
		...
		INSERT INTO [adw].[FailedMembership]
		     ...
		     SELECT
		           ...
		           ,stg.AdiKey							AS EffectiveDate
		           ,stg.mbrStg2_MbrDataUrn				AS StagingKey
		           ,stg.ClientSubscriberId				AS ClientMemberKey
		           ,stg.MstrMrnKey						AS AceID
		           ,adi.PcpNpi							AS NPI
		           ,REPLACE(adi.PcpTIN,'-','')			AS AttribTIN
		           ,stg.plnProductPlan					AS PlanName
		           ,stg.plnProductSubPlan				AS PlanID
		           ,stg.[MbrNPIFlg]						AS NPIFlgCnt
		           ,stg.[MbrPlnFlg]						AS PlnFlgCnt
		           ,stg.[MbrFlgCount]					AS PlnFlgCnt  ---- SELECT *
			FROM	ACECAREDW.ast.MbrStg2_MbrData stg
			JOIN	adi.DHTX_EligibilityMonthly adi
			ON		stg.AdiKey = adi.EligibilityMonthlyKey
			WHERE	ClientKey =  @ClientKey
			AND		EffectiveDate = @EffectiveDate
			AND		stgRowStatus <> 'Exported'
			AND		MbrNPIFlg = 0
...
/*
USAGE : EXECUTE adw.pdw_Load_FailedMembership '2021-08-01'
**/
